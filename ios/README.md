...
This SwiftUI app embeds the 밤가이 frontend in `WKWebView` and provides the native EventKit bridge required for Apple Calendar.

## Generate and run

1. Install full Xcode and XcodeGen.
2. Run `xcodegen generate` in this directory.
3. Open `Bamgai.xcodeproj`, select a Development Team, and run the `Bamgai` scheme.
