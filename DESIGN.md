...
4. 오늘 메인 — 다음 날 첫 일정, 5분 눈금으로 계산된 권장 불 끄기 구간, 추천 근거, 1분 단위 앞/뒤 조절
...
- 불 끄기 추천 시각의 분 값은 `00·05·10…55`로 제한하고, 추천을 받은 뒤에는 `−1분 / +1분` 컨트롤로 미세 조절
...
- 추천된 불 끄기 시각을 수정할 때만 `{components.time-stepper}`의 `−1분 / +1분`을 사용합니다. 추천 자체는 항상 5분 눈금(`00·05·10…55분`)에 맞추되, 사용자가 누를 때는 정확히 1분만 바뀌고 날짜가 넘어가면 날짜 문구도 함께 바뀝니다.
...
3. `{components.card-plan-night}` 안에서 5분 눈금의 추천 취침 범위와 `−1분 / +1분` 조절이 첫 3초 안에 읽히는지 테스트합니다.
